...
class SlotAttention(layers.Layer):
  ...
  def call(self, inputs):
    ...
    for _ in range(self.num_iterations):
      ...
      updates = tf.reshape(updates, [-1, self.slot_size])  # Flatten slots dimension
      slots_prev = tf.reshape(slots_prev, [-1, self.slot_size])  # Flatten slots dimension

      slots, _ = self.gru(updates, [slots_prev])

      slots = tf.reshape(slots, [-1, self.num_slots, self.slot_size])  # Restore shape
