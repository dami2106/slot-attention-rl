...
FLAGS = flags.FLAGS
flags.DEFINE_string("model_dir", "slot_attention/object_discovery/checkpoints",
                    "Where to save the checkpoints.")
flags.DEFINE_string("npy_file", "slot_attention/1000_nopick_pixel_states.npy",
                    "Path to the npy dataset.")
...
def main(argv):
    ...
    npy_file = FLAGS.npy_file
    tf.random.set_seed(FLAGS.seed)
    resolution = (128, 128)

    # Load dataset using npy instead of CLEVR.
    data_iterator = data_utils.build_npy_iterator(
        npy_file, batch_size, split="train", resolution=resolution, shuffle=True,
    ...
    ckpt = tf.train.Checkpoint(network=model, optimizer=optimizer, global_step=global_step)
